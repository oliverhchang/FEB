...
def run_analysis_for_load_case(title, node_coords, tubes, a_arm_coords, applied_force):
    ...
    # If there's only one combination, this loop will run once.
...
# --- Input Data for Rear Left Upper A-Arm (FRONT) ---
connection_node = {'x': -52.03, 'y': 9.87, 'z': 5.24}
a_arm_connection = {'x': -62.81, 'y': 21.68, 'z': 3.73}  # Upper Upright
all_tubes = [
    ("To upper aft a-arm", {'x': -62.11, 'y': 10.11, 'z': 3.67}),
    ("To bottom front a-arm", {'x': -52.18, 'y': 11.20, 'z': -4.63}),
    ("Diagonal accum protection", {'x': -22.65, 'y': 12.34, 'z': -5.50}),
    ("Top accum protection", {'x': -30.58, 'y': 12.29, 'z': 6.77}),
    ("Lateral tube", {'x': -52.03, 'y': -9.87, 'z': 5.24}),
    ("Main hoop brace", {'x': -32.31, 'y': 5.86, 'z': 34.95})
]

# Applied Forces from table
peak_compression_force = -2603.49559
peak_tension_force = 1828.079081

# --- Run Full Analysis ---
print("--- Analysis for Rear Left Upper A-Arm (FRONT) ---")
